...
export default function ProductCard({ product, wishlist = [], cart = [] }) {
  ...
  useEffect(() => {
    const storedWishlist = wishlist.length
      ? wishlist
      : getLocalStorageArray("wishlist").map(Number);
    setIsWishlisted(storedWishlist.includes(Number(product.id)));
  }, [product.id, wishlist]);

  useEffect(() => {
    const storedCart = cart.length
      ? cart
      : getLocalStorageArray("cart").map(Number);
    setIsCarted(storedCart.includes(Number(product.id)));
  }, [product.id, cart]);

  useEffect(() => {
    if (product.stock === 0) {
      const randomDays = Math.floor(Math.random() * 8) + 3;
      setRestockInDays(randomDays);
    }
  }, [product.stock]);
  ...
  const addToWishlist = async () => {
    ...
    try {
      let wishlistItems = getLocalStorageArray("wishlist").map(Number);
      if (isWishlisted) {
        await axiosInstance.delete("/wishlist", {
          data: { productId: product.id, userId },
        });
        wishlistItems = wishlistItems.filter((id) => id !== Number(product.id));
        ...
        if (!wishlistItems.includes(Number(product.id))) {
          wishlistItems.push(Number(product.id));
        }
  ...
  const addToCart = async () => {
    ...
    try {
      ...
      toast.error("Something went wrong with cart.");
    }
  };
