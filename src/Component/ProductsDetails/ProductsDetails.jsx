    ...
    (async () => {
      try {
        const { data: pRes } = await axios.get(
          `${config.API_URL}/products/id/${id}`
        );
        const p = pRes.data;
        setProduct(p);
        setImages(p.images?.map((path) => `${config.API_URL}${path}`) || []);

        const { data: rRes } = await axios.get(
          `${config.API_URL}/products/category/${p.category}`
        );
    ...
    <div className="p-6 min-h-screen bg-cream animate-fade-in">
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-12 bg-cream backdrop-blur-xl border border-white/30 rounded-[2rem] shadow-[0_10px_50px_rgba(0,0,0,0.1)] p-6 md:p-10 transition-all min-h-[600px]">
          ...
          {product.images.length > 0 ? (
            <Slider {...settings}>
              {images.map((src, i) => (
                <div
                  key={i}
                  className="w-full h-[500px] flex items-center justify-center bg-cream"
                >
                  <img
                    src={src}
                    alt={`product-${i}`}
                    className="max-h-full object-contain bg-cream"
                  />
                </div>
              ))}
            </Slider>
        ...
        <div className="text-[#333] space-y-4 overflow-y-auto max-h-[90vh] flex flex-col justify-center py-6">
          <div>
            <h1 className="text-3xl font-bold text-oranges font-marker line-clamp-2">
              {product.title}
            </h1>
            <p className="text-peach text-base line-clamp-4 mt-2">
              {product.description}
            </p>
            ...
            <div className="flex items-center gap-4 mt-4 flex-wrap">
              ...
              >
                ...
                <span className="text-sm font-medium text-peach ml-2">
                  {product.rating}
                </span>
              </div>
            </div>
